...
### Mathmatical Verification (Demo Support)

To verify that the projection is mathmatically correct, we teseted a scenario using the same inputs and formula, using excel sheets, outside of the system.
...
This matches the output generated by the application, confirming that the projection logic is correct.
